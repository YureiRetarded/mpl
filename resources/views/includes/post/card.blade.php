<div role="button" class="card mb-4"
    ...
    <div class="card-body">
        ...
        <div class="card-text mb-2">
            <a class="no-underline" href="{{route('user.project.show',['user'=>$post->project->user->name,'project'=>$post->project->link])}}">
                Проект: {{mb_strimwidth($post->project->title,0,50,'...')}}
            </a>
        </div>
        <div class="card-text mb-2">
            <a class="no-underline" target="_blank" href="{{route('user.index',['user'=>$post->project->user->name])}}">
                Автор: {{mb_strimwidth($post->project->user->name,0,50,'...')}}
            </a>
        </div>
        @if(Request::segment(1)!=='posts'&& auth()->user()!==null && auth()->user()->name===$post->project->user->name)
            <form method="POST"
